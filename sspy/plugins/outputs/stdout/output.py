...
#             raise Exception("Can't add output to %s, it is not connected to a solver" % self.GetName())
            ...
            if solver_type == "heccer":
                ...
                self._std_output.AddOutput(name, address)
    ...
    def Initialize(self):
        ...
        if not self.mode is None:
            ...
            if self.mode == 'steps':
                ...
                self._std_output.SetSteps(1)


        if not self.resolution is None:

            self._std_output.SetResolution(self.resolution)
...
#---------------------------------------------------------------------------
    ...
    def Step(self, time=None):
        ...
        """
        output_line = self._std_output.Step(time)
...
#---------------------------------------------------------------------------
    ...
    def GetData(self):
        ...
        return self._std_output.GetData()
